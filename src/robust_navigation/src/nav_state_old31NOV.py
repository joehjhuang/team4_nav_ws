...
class Navigate:
    ...
    def __init__(self):
        ...
        self.trajectory = np.array([[1.2,1.2],[1.8,1.8],[1.2,1.8],[1.2,0.6]]) #contains all waypoints as (x,y)
        self.counter=0 #iterates over all waypoints
        self.current_target=self.trajectory[0] #next waypoint
        
        self.angle_gain=0.05
        self.distance_gain=0.005
        self.vel_threshold=0.005
    	...
    	self.epsilon=0.1 #max distance to target
    ...
    def angle_callback(self,data):
        ...
        if self.pose_ready and self.angle_ready:
            ...
            #not reached target yet
                ...
                if self.theta_bearing < -m.pi:
                    self.theta_bearing = self.theta_bearing + 2*m.pi
                if self.theta_bearing > m.pi:
                    self.theta_bearing = self.theta_bearing - 2*m.pi
    ...
    def set_wheelvel(self):
        ...
        if fwdvel<= self.vel_threshold:
            fwdvel=self.vel_threshold
