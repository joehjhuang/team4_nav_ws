...
class Navigate:
    ...
    def __init__(self):
        ...
        self.arrived_sub = rospy.Subscriber("/arrived_waypoint",Bool,self.arrived_callback,queue_size=1)
        self.cmd_pub=rospy.Publisher("/cmdvel",WheelCmdVel,queue_size=1)
        self.arrived_pub=rospy.Publisher("arrived_waypoint",Bool,queue_size=1)

        self.pose_ready = False #pose data available?
        self.angle_ready = False #angle data available?  
        self.current_position=np.array([0.,0.,0.])

        #self.trajectory = np.array([[1.2,1.2],[1.8,1.8],[1.2,1.8],[0.6,1.2]]) #contains all waypoints as (x,y)
        self.current_waypoint = self.current_position
        #self.counter=0 #iterates over all waypoints
        self.current_target=-1 #next waypoint
        self.arrived = True
    ...
    def angle_callback(self,data):
		...
		#Indicate that robot has not arrived at waypoint
	...
	print('received new waypoint!')
        self.current_target=np.array([data.x,data.y])
	self.arrived_pub.publish(False)
        return
